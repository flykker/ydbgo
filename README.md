...
| 14 | **Точечный UPDATE fast-path** — `UPDATE ... WHERE pk = k` идёт напрямую через `Engine.Get`+`Engine.Update` вместо полного скана; диагностика зависаний (это был raft-replay старых full-scan UPDATE из лога после рестартов, а не точечный путь) | ✅ сделано, проде: UPDATE point 2708 ops/s, DELETE point 2588 ops/s (5 узлов, RF=3, 1M строк) |
| 15 | **Фикс проекции `SELECT <одна колонка>`** — `scanShardProjected` дважды раскладывал строки в полную ширину (`expandProjected` поверх уже полных `rowsFromPayload`), из-за чего `SELECT g WHERE id=…` возвращал пусто; убран повторный вызов, удалена мёртвая функция | ✅ сделано, регрессия в `TestShardedMpart` (проекция последней колонки) |
| 16 | **Columnar range-UPDATE (модель мутаций ClickHouse)** — `UPDATE ... WHERE <PK-диапазон>` на `CSTORE` читает только строки диапазона колоночно (одна транзакция: `ScanColumns(rng)` + батч-upsert) вместо полного скана таблицы с по-строчными `Engine.Update`; изучены исходники ClickHouse (`MutateTask`, `MutationsInterpreter`: перезапись только затронутых колонок, hardlink нетронутых, пропуск гранул) | ✅ сделано, `TestShardedRangeUpdate`; UPDATE range 10k: 54s → **0.24s**; 100k: таймаут → **~1.85s** (CH: 0.153s); DELETE range 100k ~0.14s **быстрее CH** (0.218s) |
| 17 | **Референсные исходники в репо** — `source/clickhouse` (MutateTask, MutationsInterpreter, MergeTree) и `source/ydb` (columnshard, tablet_flat, engine) — shallow+sparse клоны для чтения «прямо из репо»; отдельный go.mod (граница модуля) и `.gitignore` | ✅ сделано |

### Что дальше (roadmap)

- [ ] **UPDATE пишет только затронутые колонки (CH hardlink)** — сейчас `execUpdateColumnar` перезаписывает все колонки диапазона; по образцу CH читать/переписывать только SET-колонки, остальные копировать. Цель: range-UPDATE 100k ~1.85s → ~0.3s.
- [ ] **Проверить просадку UPDATE point** (2708 → ~1500 ops/s после реструктуризации `execUpdate`): убедиться, что это не регрессия, а шум/накопление частей.
- [ ] **Изучить YDB columnshard** (`source/ydb`) — сравнить модель обновлений/компакции, взять лучший опыт.
- [ ] **Расширить A/B vs ClickHouse** — range SELECT/LIMIT/агрегаты, footprint на диске, light-delete маска CH.
